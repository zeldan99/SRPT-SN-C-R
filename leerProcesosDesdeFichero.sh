...
# =============================================
# Fichero: leerProcesosDesdeFichero.sh
# Descripcion: Lee el fichero de datos de entrada y almacena los datos leidos en una serie de arrays.
# Los datos del fichero vienen separados por ":" conforme al esquema siguiente:
# La primera linea representa el tamano de la memoria, luego cada linea siguiente representa un proceso:
#               t.llegada : t.ejecución : memoria
...
# Globales: $ficheroDatosEntrada
leerFichero() {
  local n=-1
  while read line; do
    # Convertimos el registro leído en un array de la siguiente forma
    # lineArray[0]=  t.llegada
    # lineArray[1]=  t.ejecución
    # lineArray[2]=  t.memoria
    lineArray=($line)

    # Guardamos cada campo en su array correspondiente
    if [[ $n -eq -1 ]]; then
      mem_total=${lineArray[0]} # primera linea fichreo tamano total de memoria
      mem_total1=${lineArray[0]}
    else
      entradas[$n]=${lineArray[0]}  # lineArray[0]=  t.llegada
      ejecucion[$n]=${lineArray[1]} # lineArray[1]=  t.ejecución
      tamemory[$n]=${lineArray[2]}  # lineArray[2]=  t.memoria
...
read ok
if ! [ "$ok" == "s" ] && ! [ "$ok" == "" ]; then
  cecho "Programa cancelado, modifique los valores desde entrada.dat y reinicie." $FRED
  exit 0
fi
cecho "---------------------------------------------" $FRED
...
cecho "Estos son los datos de partida:" $FBLE >>salidacolor.txt
printf "\n" >>salida.txt

cecho "-----------------------------------------------------------------" $FYEL >>salidacolor.txt
cecho "|    PROCESOS   |    T.LLEG.    |     T.EJEC.   |     MEMORIA   |" $FYEL >>salidacolor.txt
cecho "-----------------------------------------------------------------" $FYEL >>salidacolor.txt
...
cecho "MEMORIA TOTAL: $mem_total M" $FCYN >>salidacolor.txt
