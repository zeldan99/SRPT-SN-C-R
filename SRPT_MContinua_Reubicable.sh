...
#
# Autores : Zeldan Javier Campos, Miguel Arroyo Pérez y Adrián Pineda Miñón
# Fecha : 23/04/2022
#
# =============================================
...
cecho "**********************************************************************************" >> salidacolor.txt $FCYN
cecho "*                  SISTEMAS OPERATIVOS - PRÁCTICA DE CONTROL                     *" >> salidacolor.txt $FCYN
cecho "*                               Curso 2019-20                                    *" >> salidacolor.txt $FCYN
cecho "*                                                                                *" >> salidacolor.txt $FCYN
cecho "*                                                                                *" >> salidacolor.txt $FCYN
cecho "*              SRPT-SEGÚN NECESIDADES-MEMORIA CONTINUA-REUBICABLE                *" >> salidacolor.txt $FCYN
cecho "*                                                                                *" >> salidacolor.txt $FCYN
cecho "*                                                                                *" >> salidacolor.txt $FCYN
cecho "*                              Alumnos nuevos:                                   *" >> salidacolor.txt $FCYN
cecho "*                          · Daniel Puente Ramírez                               *" >> salidacolor.txt $FCYN
cecho "*                                                                                *" >> salidacolor.txt $FCYN
cecho "**********************************************************************************" >> salidacolor.txt $FCYN
...
while [ $num -ne 3 ] && [ "$continuar" == "SI" ]
do
  read num
  case $num in
   "1" )
         # Cargamos los datos por teclado
         source readprocesos.sh
         continuar=NO
         ;;
    ...
    do
       ...
       then
        ...
        if [[ "$imprimirT" == "NO" ]]; then
          ...
          echo " " >> salidacolor.txt
          cecho " T = $tiempo            MT = $memVacia" >> salidacolor.txt $FYEL
          imprimirT="SI"
        fi
         ...
         cecho " El proceso ${ordenEntrada[$i]} ha entrado en la cola." >> salidacolor.txt $FWHT
         fi
...
do
    ...
    then
        ...
        if [[ "$imprimirT" == "NO" ]]; then
         ...
         cecho " El proceso ${ordenEntrada[$i]} ha terminado su ejecución. $mem_libre M restantes" >> salidacolor.txt $FBLE
         imprimirYa="SI"
     fi
     cpu_ocupada=NO
...
do
   ...
   if [[ $mem_libre -lt "0" ]] ; then
    reubica=0
      if [[ ${evento[$tiempo]} -eq 1 ]] ; then
       ...
       cecho " El proceso ${ordenEntrada[$i]} no cabe en memoria en este momento." >> salidacolor.txt $FRED
       imprimirYa="SI"
   fi
   mem_libre=`expr $mem_libre + ${tamemoryAuxiliar[$i]}`
      ...
      if [[ ${evento[$tiempo]} -eq 1 ]] ; then
        ...
        if [[ "$imprimirT" == "NO" ]]; then
         ...
         cecho " Se bloquean todos los procesos siguientes, para que el siguiente en entrar a memoria sea ${ordenEntrada[$i]}." >> salidacolor.txt $FRED
         imprimirYa="SI"
     fi
  ...
  if [[ ${evento[$tiempo]} -eq 1 ]] ; then
    ...
    #Buscamos el hueco donde lo vamos a meter
     ...
     cecho " El proceso ${ordenEntrada[$i]} entra en memoria. $mem_libre M restante." >> salidacolor.txt $FCYN
     imprimirYa="SI"
 fi
 enmemoria[$i]=1
 realizadoAntes=0
    ...
    do
        if [[ ${enmemoria[$i]} -eq 1 ]] && [[ ${ejecucionAuxiliar[$i]} -lt ${tejecucion[$i]} ]] && [[ ${enejecucion[$i]} -eq 0 ]] ; then
          ...
          imprimirT="SI"
              ...
              cecho " El proceso ${ordenEntrada[$i]} está pausado." >> salidacolor.txt $FMAG
              imprimirYa="SI"
          fi
      fi
  done
...
echo " " >> salidacolor.txt
cecho " Tiempo: $tiempofinal  " >> salidacolor.txt $FYEL
cecho " Ejecución terminada." >> salidacolor.txt $FMAG
